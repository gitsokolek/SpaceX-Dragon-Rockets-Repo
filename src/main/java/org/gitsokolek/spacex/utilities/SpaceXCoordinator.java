...
public class SpaceXCoordinator
{
	...
	public Mission assignDragon(MissionId missionId, DragonId dragonId)
	{
		...
		if (m.getStatus() == MissionStatus.ENDED)
		{
			throw new IllegalStateException("Mission already ended");
		}
		requireDragon(dragonId);

		Optional<Mission> other = findMissionByDragon(dragonId);
		if (other.isPresent() && !other.get().getId().equals(missionId))
		{
			throw new IllegalStateException("Dragon already assigned to another mission");
		}
	...
	private MissionStatus computeStatusForMission(Mission m)
	{
		...
		for (DragonId did : m.getAssignedDragons())
		{
			...
			if (od.isPresent())
			{
				...
				if (s == DragonStatus.IN_REPAIR)
				{anyInRepair = true;}
				if (s == DragonStatus.IN_SPACE)
				{anyInSpace = true;}
			}
		}
	...
	private Optional<Mission> findMissionByDragon(DragonId id)
	{
		...
		for (Mission m : all)
		{
			if (m.getAssignedDragons().contains(id))
